  ...
  return (
    <div className="app-card p-5 space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <div className="rounded-xl border border-app-border p-3 bg-white/5">
          <p className="text-xs text-app-text-secondary">Flows</p>
          <p className="text-lg font-bold text-app-text mt-1">{metrics?.crossBorderFlowCount ?? 0}</p>
        </div>
        <div className="rounded-xl border border-app-border p-3 bg-white/5">
          <p className="text-xs text-app-text-secondary">Settled</p>
          <p className="text-lg font-bold text-app-text mt-1">{metrics?.settledCrossBorderCount ?? 0}</p>
        </div>
        <div className="rounded-xl border border-app-border p-3 bg-white/5">
          <p className="text-xs text-app-text-secondary flex items-center gap-1"><Timer className="w-3 h-3" /> Avg Latency</p>
          <p className="text-lg font-bold text-app-text mt-1">{metrics?.averageLatencySeconds ?? 0}s</p>
        </div>
        <div className="rounded-xl border border-app-border p-3 bg-white/5">
          <p className="text-xs text-app-text-secondary flex items-center gap-1"><Fuel className="w-3 h-3" /> Avg Fee</p>
          <p className="text-lg font-bold text-app-text mt-1">{(metrics?.averageSettlementFeeCelo ?? 0).toFixed(6)} CELO</p>
        </div>
      </div>
      ...
      <div className="space-y-2">
        {(metrics?.routes ?? []).slice(0, 5).map((route) => (
          <div key={`${route.intentId}-${route.route}`} className="rounded-lg border border-app-border p-3 bg-white/5 flex items-center justify-between">
            <div>
              <p className="text-sm font-semibold text-app-text">{route.route}</p>
              <p className="text-xs text-app-text-secondary">Intent #{route.intentId} · {route.stablePair}</p>
            </div>
            <div className="text-right">
              <p className="text-sm font-bold text-app-text">{route.valueCusd.toFixed(4)} cUSD</p>
